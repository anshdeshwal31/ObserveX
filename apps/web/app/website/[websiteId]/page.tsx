...
import { getWebsiteStatus, updateWebsiteThresholds, getIncidents, getIncidentDetails, type WebsiteInfo, type WebsiteTick, type Incident, type IncidentEvent } from "../../../lib/api";
    ...
    async function load() {
      try {
        ...
        if (token) {
          ...
          try {
            const allIncidents = await getIncidents(token);
            const siteIncidents = allIncidents.filter(i => i.website_id === websiteId);
            const latestOpen = siteIncidents.find(i => i.status === "Open" || i.status === "Acknowledged");
